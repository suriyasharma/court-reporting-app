...
import ReporterDash from './ReporterDash'
...
export default function AuthenticatedApp({ user, onLogout }) {
  const [invoices, setInvoices, invLoaded]       = useSupabaseCollection('invoices',  [], { toDb: invoiceToDb,  fromDb: invoiceFromDb,  localKey: 'cr_invoices' })
  const [reporters, setReporters, repLoaded]     = useSupabaseCollection('reporters', [], { toDb: reporterToDb, fromDb: reporterFromDb, localKey: 'cr_reporters' })
  const [admins, setAdmins, admLoaded]           = useSupabaseCollection('admins', initAdmins, { toDb: adminToDb, fromDb: adminFromDb, localKey: 'cr_admins' })
  const [jobs, setJobs, jobsLoaded]              = useSupabaseCollection('jobs', [], { pkField: 'deposition_id', toDb: jobToDb, fromDb: jobFromDb, localKey: 'cr_jobs' })
  const [auditLog, setAuditLog, auditLoaded]     = useSupabaseCollection('audit_log', [], { toDb: auditToDb, fromDb: auditFromDb, localKey: 'cr_audit' })
